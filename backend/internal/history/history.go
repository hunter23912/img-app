...
type History = imageHistory

func New() *History { return newImageHistory() }

const MaxImageHistory = maxImageHistory

func NormalizeImageURL(raw string) (string, bool) { return normalizeHistoryImageURL(raw) }
func ImagePath(id string) string                  { return historyImagePath(id) }
func ImageReference(id, image string) string      { return historyImageReference(id, image) }
func ParseImagePath(path string) (string, bool)   { return parseHistoryImagePath(path) }
func IsBase64ImageDataURL(value string) bool      { return isBase64ImageDataURL(value) }
...
func normalizeHistoryImageURL(raw string) (string, bool) {
	...
	// Some image providers return b64_json, which the relay converts to a data
	// URL. Keep these results in the database as well as HTTPS image URLs.
